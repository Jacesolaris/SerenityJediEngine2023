...
void bot_update_input(bot_state_t* bs, const int time, const int elapsed_time)
{
	...
	if (bs->cur_ps.weapon == WP_SABER)
	{
		if (bs->currentEnemy
			...
			&& (VectorDistance(g_entities[bs->cur_ps.client_num].r.currentOrigin, bs->currentEnemy->r.currentOrigin) < 300 || walktime[bs->cur_ps.client_num] > level.time))
		{
...
int pass_loved_one_check(const bot_state_t* bs, const gentity_t* ent)
{
	...
	while (i < bs->lovednum)
	{
		if (strcmp(level.clients[loved->client].pers.netname, bs->loved[i].name) == 0)
		{
			...
			if (is_teamplay() && !OnSameTeam(&g_entities[bs->client], &g_entities[loved->client]) && bs->loved[i].level < 2)
			{
