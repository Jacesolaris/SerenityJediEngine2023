...
qboolean npc_is_light_jedi(gentity_t* self)
{
	switch (self->client->NPC_class)
	{
	case CLASS_JEDI:
		...
		for (i = 0; i < num_ents; i++)
		{
			...
			if (radiusEnt->client == NULL)
			{
				...
				if (G_EntIsBreakable(radiusEnt->s.number))
				{
					...
					G_Damage(radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin, radiusEnt->r.currentOrigin, 100, 0,
					         MOD_ROCKET_SPLASH);
				}
				continue;
			}
			...
			if (dist <= radius)
			{
				if (dist < halfRad)
				{
					...
					G_Damage(radiusEnt, NPCS.NPC, NPCS.NPC, vec3_origin, radiusEnt->r.currentOrigin, Q_irand(20, 30),
...
void Boba_SetJetpackAnims(gentity_t* self)
{
	vec3_t angs, mvnt;
...
void Boba_SetBolts(gentity_t* self)
{
	if (self && self->client)
	{
			...
			else
			{
				...
				else if (enemyInFOV)
				{
					...
					if (hit == NPCS.NPC->enemy->s.number
						...
						|| hitEnt && hitEnt->takedamage && (hitEnt->r.svFlags & SVF_GLASS_BRUSH || hitEnt->health <
...
void NPC_Jedi_RateNewEnemy(gentity_t* self, gentity_t* enemy)
{
	...
	case WP_BLASTER:
		...
		else
		{
			...
			weaponAggression = 5; //hang back for a second
		}
		break;
	default:
		healthAggression = (float)self->health / 200.0f * 8.0f;
		weaponAggression = 9; //approach
		break;
	}
	//Average these with current aggression
	const int newAggression = ceil((healthAggression + weaponAggression + (float)self->NPC->stats.aggression) / 3.0f);
...
int Jedi_ReCalcParryTime(gentity_t* self, const evasionType_t evasionType)
{
	if (!self->client)
	{
		return 0;
...
void Jedi_CheckJumpEvasionSafety(gentity_t* self, usercmd_t* cmd, const evasionType_t evasionType)
{
...
==========================================================================================
*/
gentity_t* Jedi_FindEnemyInCone(gentity_t* self, gentity_t* fallback, const float minDot)
{
