...
static void ProcessMoveCommands(Vehicle_t* pVeh)
{
...
#elif _CGAME
					trap->S_StartSound(NULL, pm->ps->client_num, CHAN_LOCAL, pVeh->m_pVehicleInfo->soundHyper);
#endif
				}
	...
	const qboolean isLandingOrLaunching = FighterIsLanding(pVeh, parentPS) || FighterIsLaunching(pVeh, parentPS);
	...
	float speedInc = pVeh->m_pVehicleInfo->acceleration * pVeh->m_fTimeModifier;
	...
	float speedIdleDec = pVeh->m_pVehicleInfo->decelIdle * pVeh->m_fTimeModifier;
	const float speedIdle = pVeh->m_pVehicleInfo->speedIdle;
	const float speedIdleAccel = pVeh->m_pVehicleInfo->accelIdle * pVeh->m_fTimeModifier;
...
void FighterYawAdjust(const Vehicle_t* pVeh, const playerState_t* riderPS, const playerState_t* parentPS)
{
	...
	{
		float s = parentPS->speed;
		const float maxDif = pVeh->m_pVehicleInfo->turningSpeed * 0.8f; //magic number hackery
...
void FighterPitchAdjust(const Vehicle_t* pVeh, const playerState_t* riderPS, const playerState_t* parentPS)
{
	...
	if (parentPS && parentPS->speed)
	{
		...
		pVeh->m_vOrientation[PITCH] = AngleNormalize360(
			pVeh->m_vOrientation[PITCH] - angDif * (pVeh->m_fTimeModifier * 0.2f));
	}
}
...
static void ProcessOrientCommands(Vehicle_t* pVeh)
{
	...
	else
	{
		...
		const float strafeRoll = parentPS->hackingTime / MAX_STRAFE_TIME * pVeh->m_pVehicleInfo->rollLimit;
...
static void AnimateVehicle(Vehicle_t* pVeh)
{
	...
	{
		const qboolean isLanding = FighterIsLanding(pVeh, parentPS);
		const qboolean isLanded = FighterIsLanded(pVeh, parentPS);
