...
qboolean PM_SaberKataDone(const int curmove, const int newmove)
{
	...
	if (pm->ps->fd.saberAnimLevel == FORCE_LEVEL_3)
	{
		...
		{
			if (pm->ps->fd.saberAnimLevel >= FORCE_LEVEL_3 && pm->ps->saberAttackChainCount > Q_irand(
	...
	else
	{
		...
		if ((pm->ps->fd.saberAnimLevel == FORCE_LEVEL_2 || pm->ps->fd.saberAnimLevel == SS_DUAL) && pm->ps->
...
int PM_SaberLockResultAnim(playerState_t* duelist, const qboolean superBreak, const qboolean won)
{
	...
	{
		//other guy
		NPC_SetAnim(&g_entities[duelist->clientNum], SETANIM_BOTH, base_anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
	}
#else
	PM_SetAnim(SETANIM_BOTH, base_anim, SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD);
#endif
...
qboolean PM_CheckUpsideDownAttack(void)
{
	...
	{
		const float animLength = PM_AnimLength(0, pm->ps->legsAnim);
...
weapChecks:
	...
	if (pm->ps->weaponstate == WEAPON_RAISING)
	{
		...
		else
		{
...
#endif
			{
				...
				{
					PM_SetAnim(SETANIM_TORSO, PM_ReadyPoseForSaberAnimLevelPlayer(), SETANIM_FLAG_NORMAL);
				}
			}
		}
	...
	if (!delayed_fire)
	{
		...
		if (g_entities[pm->ps->clientNum].r.svFlags & SVF_BOT) //npc
		{
			...
			if (curmove == LS_A_JUMP_T__B_ || curmove == LS_A_JUMP_PALP_ || pm->ps->torsoAnim == BOTH_FORCELEAP2_T__B_
...
#endif
		{
			...
			// check for fire
			else if (!(pm->cmd.buttons & BUTTON_ATTACK) || HoldingBlock || ActiveBlocking)
			{
				//not attacking
				pm->ps->weaponTime = 0;
...
void PM_SaberFatigue(playerState_t* ps, const int newMove, const int anim)
{
	if (ps->saberMove != newMove)
	{
		//wasn't playing that attack before
...
extern float bg_get_torso_anim_point(const playerState_t* ps, int AnimIndex);

qboolean BG_SaberInFullDamageMove(const playerState_t* ps, const int AnimIndex)
{
	//The player is attacking with a saber attack that does full damage
	const float TorsoAnimPoint = bg_get_torso_anim_point(ps, AnimIndex);
		...
		|| PM_SuperBreakWinAnim(ps->torsoAnim))
	{
		...
		if ((ps->saberMove == LS_A_FLIP_STAB || ps->saberMove == LS_A_FLIP_SLASH
			...
			&& (TorsoAnimPoint >= 0.30f && TorsoAnimPoint <= 0.95f)) //assumes that the dude is
		{
		...
		if ((ps->saberMove == BOTH_ROLL_STAB
			|| ps->saberMove == LS_ROLL_STAB) && (TorsoAnimPoint >= 0.30f && TorsoAnimPoint <= 0.95f))
		{
		...
		if ((ps->saberMove == BOTH_STABDOWN || ps->saberMove == BOTH_STABDOWN_STAFF || ps->saberMove ==
			...
			LS_STABDOWN_DUAL)
			&& (TorsoAnimPoint >= 0.35f && TorsoAnimPoint <= 0.95f))
		{
...
qboolean BG_SaberInPartialDamageMove(const playerState_t* ps, const int AnimIndex)
{
...
qboolean BG_SaberInNonIdleDamageMove(const playerState_t* ps, const int AnimIndex)
{
