...
itemconfig_t* LoadItemConfig(char* filename)
{
	...
	itemconfig_t* ic = static_cast<itemconfig_t*>(GetClearedHunkMemory(sizeof(itemconfig_t) +
...
int* ItemWeightIndex(weightconfig_t* iwc, itemconfig_t* ic)
{
	...
	int* index = static_cast<int*>(GetClearedMemory(sizeof(int) * ic->numiteminfo));
...
void BotInitInfoEntities(void)
{
	...
	for (int ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		{
			maplocation_t* ml = static_cast<maplocation_t*>(GetClearedMemory(sizeof(maplocation_t)));
		...
		{
			campspot_t* cs = static_cast<campspot_t*>(GetClearedMemory(sizeof(campspot_t)));
...
// Changes Globals:		-
//===========================================================================
int BotGetLevelItemGoal(const int index, char* name, bot_goal_t* goal)
{
...
{
	const vec3_t mins = { -8, -8, -8 }, maxs = { 8, 8, 8 };
