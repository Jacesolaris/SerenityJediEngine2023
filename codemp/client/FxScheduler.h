...
public:
	...
	void PlayEffect(int id, vec3_t org, vec3_t fwd, int vol = -1, int rad = -1, bool isPortal = false);
	...
	void PlayEffect(const char* file, vec3_t org, vec3_t fwd, int vol = -1, int rad = -1);
	// builds arbitrary perp. right vector, does a cross product to define up
