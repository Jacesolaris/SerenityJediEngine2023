	...
	while (*psText) {
		...
		if (uiLetter == '\n') {
			...
			if (r_aspectCorrectFonts->integer == 1) {
				fValue *= static_cast<float>((SCREEN_WIDTH * glConfig.vidHeight)) / static_cast<float>((SCREEN_HEIGHT * glConfig.vidWidth));
			}
...
void R_ReloadFonts_f(void)
{
	...
	if (iFontToFind == g_iCurrentFontIndex) // found all of them?
	{
		...
		{
#ifdef _DEBUG
			const int iNewFontHandle = RE_RegisterFont(vstrFonts[iFont].c_str());
