...
unsigned int bitget(int n)
{
	...
	bitdat.bits -= n;
	const unsigned int x = bitdat.bitbuf >> bitdat.bits;
...
static void unpack_main(unsigned char* pcm, int igr)
{
	...
	for (ch = 0; ch < pMP3Stream->nchan; ch++)
	{
		...
		const int bit0 = pMP3Stream->main_pos_bit & 7;
		if (bit0)
			bitget(bit0);
		...
		int n3 = side_info.gr[igr][ch].big_values;
		n3 = n3 + n3;
...
IN_OUT L3audio_decode_MPEG1(unsigned char* bs, unsigned char* pcm)
{
	...
	/*-- unpack side info --*/
	const int side_bytes = unpack_side_MPEG1();
	...
	const int nbytes = padframebytes - side_bytes - pMP3Stream->crcbytes;
...
IN_OUT L3audio_decode_MPEG2(unsigned char* bs, unsigned char* pcm)
{
	...
	/*-- unpack side info --*/
	const int side_bytes = unpack_side_MPEG2(igr);
	const int padframebytes = pMP3Stream->framebytes + pMP3Stream->pad;
	in_out.in_bytes = padframebytes;
