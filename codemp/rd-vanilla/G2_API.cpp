...
void RestoreGhoul2InfoArray()
{
	if (singleton == nullptr)
	{
		...
		const size_t read =
#endif
			singleton->Deserialize(static_cast<const char*>(data), size);
		Z_Free((void*)data);
#ifdef _DEBUG
		assert(read == size);
#endif
...
qboolean G2API_SetLodBias(CGhoul2Info* ghl_info, const int lodBias)
{
...
qboolean G2API_SetSurfaceOnOff(CGhoul2Info_v& ghoul2, const char* surfaceName, const int flags)
{
	...
	if (G2_SetupModelPointers(ghl_info))
	{
		...
		return G2_SetSurfaceOnOff(ghl_info, ghl_info->mSlist, surfaceName, flags);
	}
	return qfalse;
}

int G2API_GetSurfaceOnOff(CGhoul2Info* ghl_info, const char* surfaceName)
{
	if (G2_SetupModelPointers(ghl_info))
	{
		return G2_IsSurfaceOff(ghl_info, ghl_info->mSlist, surfaceName);
	}
	return -1;
}

qboolean G2API_SetRootSurface(CGhoul2Info_v& ghoul2, const int model_index, const char* surfaceName)
{
	if (G2_SetupModelPointers(ghoul2))
	{
		return G2_SetRootSurface(ghoul2, model_index, surfaceName);
	}

	return qfalse;
}

int G2API_AddSurface(CGhoul2Info* ghl_info, const int surfaceNumber, const int polyNumber, const float BarycentricI, const float BarycentricJ,
                     const int lod)
{
	if (G2_SetupModelPointers(ghl_info))
	{
		// ensure we flush the cache
		ghl_info->mMeshFrameNum = 0;
		return G2_AddSurface(ghl_info, surfaceNumber, polyNumber, BarycentricI, BarycentricJ, lod);
	}
	return -1;
}
...
int G2API_GetSurfaceRenderStatus(CGhoul2Info_v& ghoul2, const int model_index, const char* surfaceName)
{
	CGhoul2Info* ghl_info = &ghoul2[model_index];

	if (G2_SetupModelPointers(ghl_info))
	{
		return G2_IsSurfaceRendered(ghl_info, surfaceName, ghl_info->mSlist);
	}
	return -1;
}
...
qboolean G2API_DoesBoneExist(CGhoul2Info_v& ghoul2, const int model_index, const char* boneName)
{
	...
	{
		//model is valid
		mdxaHeader_t* mdxa = ghl_info->currentModel->mdxa;
		if (mdxa)
		{
...
qboolean G2API_SetBoneAnimIndex(CGhoul2Info* ghl_info, const int index, const int AstartFrame, const int AendFrame,
	const int flags, const float animSpeed, const int current_time, const float AsetFrame,
	const int blend_time)
{
	...
	if (res)
	{
		...
		return G2_Set_Bone_Anim_Index(ghl_info->mBlist, index, start_frame, end_frame, flags, animSpeed, current_time,
...
qboolean G2API_SetBoneAnim(CGhoul2Info_v& ghoul2, const int model_index, const char* boneName, const int AstartFrame,
	const int AendFrame, const int flags, const float animSpeed, const int current_time,
	const float AsetFrame, const int blend_time)
{
	int end_frame = AendFrame;
	int start_frame = AstartFrame;
	float setFrame = AsetFrame;
	...
	int* start_frame, int* end_frame, int* flags, float* animSpeed, int* modelList)
{
	...
	if (G2_SetupModelPointers(ghl_info))
	{
		...
		const qboolean ret = G2_Get_Bone_Anim(ghl_info, ghl_info->mBlist, boneName, aCurrentTime, current_frame,
			start_frame, end_frame, flags, animSpeed, modelList, ghl_info->mModelindex);
#ifdef _DEBUG
		/*
		assert(*end_frame>0);
		assert(*end_frame<100000);
...
qboolean G2API_PauseBoneAnim(CGhoul2Info* ghl_info, const char* boneName, const int current_time)
{
	if (G2_SetupModelPointers(ghl_info))
	{
		return G2_Pause_Bone_Anim(ghl_info, ghl_info->mBlist, boneName, current_time);
	}
	return qfalse;
}
...
qboolean G2API_StopBoneAnim(CGhoul2Info* ghl_info, const char* boneName)
{
	if (G2_SetupModelPointers(ghl_info))
	{
		return G2_Stop_Bone_Anim(ghl_info->mFileName, ghl_info->mBlist, boneName);
	}
	return qfalse;
}
	...
	qhandle_t* modelList, const int blend_time, const int current_time)
{
	...
	if (res)
	{
		// ensure we flush the cache
		ghl_info->mSkelFrameNum = 0;
		return G2_Set_Bone_Angles_Index(ghl_info->mBlist, index, angles, flags, yaw, pitch, roll, modelList,
			ghl_info->mModelindex, blend_time, current_time);
	}
	return qfalse;
}
	...
	qhandle_t* modelList, const int blend_time, const int current_time)
{
	if (ghoul2.size() > model_index)
	{
		...
		if (res)
		{
			// ensure we flush the cache
			ghl_info->mSkelFrameNum = 0;
			return G2_Set_Bone_Angles(ghl_info, ghl_info->mBlist, boneName, angles, flags, up, left, forward, modelList,
				ghl_info->mModelindex, blend_time, current_time);
		}
	}
	return qfalse;
...
qboolean G2API_SetBoneAnglesMatrixIndex(CGhoul2Info* ghl_info, const int index, const mdxaBone_t& matrix,
	const int flags, qhandle_t* modelList, const int blend_time, const int current_time)
{
	if (G2_SetupModelPointers(ghl_info))
	{
		// ensure we flush the cache
		ghl_info->mSkelFrameNum = 0;
		return G2_Set_Bone_Angles_Matrix_Index(ghl_info->mBlist, index, matrix, flags, modelList, ghl_info->mModelindex,
	...
	const int flags, qhandle_t* modelList, const int blend_time, const int current_time)
{
	if (G2_SetupModelPointers(ghl_info))
	{
		// ensure we flush the cache
		ghl_info->mSkelFrameNum = 0;
		return G2_Set_Bone_Angles_Matrix(ghl_info->mFileName, ghl_info->mBlist, boneName, matrix, flags, modelList,
			ghl_info->mModelindex, blend_time, current_time);
	}
	return qfalse;
}
...
qboolean G2API_StopBoneAngles(CGhoul2Info* ghl_info, const char* boneName)
{
	if (G2_SetupModelPointers(ghl_info))
	{
		// ensure we flush the cache
		ghl_info->mSkelFrameNum = 0;
		return G2_Stop_Bone_Angles(ghl_info->mFileName, ghl_info->mBlist, boneName);
	}
	return qfalse;
}
...
int G2_Find_Bone_Rag(const CGhoul2Info* ghl_info, const boneInfo_v& blist, const char* boneName);
...
static inline boneInfo_t* G2_GetRagBoneConveniently(CGhoul2Info_v& ghoul2, const char* boneName)
{
	...
	const int bone_index = G2_Find_Bone_Rag(ghl_info, ghl_info->mBlist, boneName);
...
qboolean G2API_GetRagBonePos(CGhoul2Info_v& ghoul2, const char* boneName, vec3_t pos, vec3_t entAngles, vec3_t entPos,
	vec3_t entScale)
{
	//do something?
	return qfalse;
...
qboolean G2_SetBoneIKState(CGhoul2Info_v& ghoul2, int time, const char* boneName, int ikState,
...
int G2API_AddBolt(CGhoul2Info_v& ghoul2, const int model_index, const char* boneName)
{
	...
	if (ghoul2.size() > model_index)
	{
		...
		if (G2_SetupModelPointers(ghl_info))
		{
			return G2_Add_Bolt(ghl_info, ghl_info->mBltlist, ghl_info->mSlist, boneName);
		}
	}
	return -1;
}

int G2API_AddBoltSurfNum(CGhoul2Info* ghl_info, const int surfIndex)
{
	if (G2_SetupModelPointers(ghl_info))
	{
		return G2_Add_Bolt_Surf_Num(ghl_info, ghl_info->mBltlist, ghl_info->mSlist, surfIndex);
	}
	return -1;
}

qboolean G2API_AttachG2Model(CGhoul2Info_v& ghoul2From, const int modelFrom, CGhoul2Info_v& ghoul2To, int toBoltIndex,
	int toModel)
{
...
qboolean G2API_AttachEnt(int* boltInfo, CGhoul2Info_v& ghoul2, const int model_index, int toBoltIndex, int entNum,
	int toModelNum)
{
...
static inline bool G2_NeedRetransform(CGhoul2Info* g2, const int frameNum)
{
...
int G2API_CopyGhoul2Instance(CGhoul2Info_v& g2From, CGhoul2Info_v& g2To, const int model_index)
{
...
void G2API_DuplicateGhoul2Instance(CGhoul2Info_v& g2From, CGhoul2Info_v** g2To)
{
	...
	*g2To = new CGhoul2Info_v;
	...
	/*ignore = */
	G2API_CopyGhoul2Instance(g2From, ghoul2, -1);
}
...
char* G2API_GetSurfaceName(CGhoul2Info_v& ghoul2, const int model_index, const int surfNumber)
{
	...
	if (G2_SetupModelPointers(ghl_info))
	{
		auto mod = (model_t*)ghl_info->currentModel;
		mdxmSurface_t* surf;
...
int G2API_GetSurfaceIndex(CGhoul2Info* ghl_info, const char* surfaceName)
{
	if (G2_SetupModelPointers(ghl_info))
	{
		return G2_GetSurfaceIndex(ghl_info, surfaceName);
	}
	return -1;
}
...
qboolean G2_TestModelPointers(CGhoul2Info* ghl_info) // returns true if the model is properly set up
{
	...
	if (ghl_info->mModelindex != -1)
	{
		...
		if (ghl_info->currentModel)
		{
			if (ghl_info->currentModel->mdxm)
			{
				if (ghl_info->currentModelSize)
				{
					if (ghl_info->currentModelSize != ghl_info->currentModel->mdxm->ofsEnd)
					{
						Com_Error(ERR_DROP, "Ghoul2 model was reloaded and has changed, map must be restarted.\n");
					}
				}
				ghl_info->currentModelSize = ghl_info->currentModel->mdxm->ofsEnd;
				ghl_info->animModel = R_GetModelByHandle(ghl_info->currentModel->mdxm->animIndex);
...
qboolean G2_SetupModelPointers(CGhoul2Info* ghl_info) // returns true if the model is properly set up
{
	...
	if (ghl_info->mModelindex != -1)
	{
		...
		if (ghl_info->currentModel)
		{
			G2ERROR(ghl_info->currentModel->mdxm, va("Model has no mdxm (glm) %s", ghl_info->mFileName));
			if (ghl_info->currentModel->mdxm)
			{
				if (ghl_info->currentModelSize)
				{
