...
int G2API_AddSurface(CGhoul2Info* ghlInfo, const int surfaceNumber, const int polyNumber, const float BarycentricI, const float BarycentricJ,
                     const int lod)
{
	if (G2_SetupModelPointers(ghlInfo))
	{
...
qboolean G2API_HasGhoul2ModelOnIndex(CGhoul2Info_v** ghlRemove, const int model_index)
{
	...
	if (!ghlInfo.size() || ghlInfo.size() <= model_index || ghlInfo[model_index].mModelindex == -1)
	{
		return qfalse;
	}
...
qboolean G2API_RemoveGhoul2Model(CGhoul2Info_v** ghlRemove, const int model_index)
{
	...
	if (ghlInfo.size() > model_index)
	{
		...
		if (ghlInfo[model_index].mBoneCache)
		{
		...
		ghlInfo[model_index].mBlist.clear();
		ghlInfo[model_index].mBltlist.clear();
		ghlInfo[model_index].mSlist.clear();
...
qboolean G2API_RemoveGhoul2Models(CGhoul2Info_v** ghlRemove)
{
	...
	for (int model_index = 0; model_index < ghlInfo.size(); model_index++)
	{
		if (ghlInfo[model_index].mModelindex == -1)
		{
			continue;
		}
#ifdef _G2_GORE
	...
	const int blend_time)
{
	...
	if (res)
	{
		...
		return G2_Set_Bone_Anim_Index(ghlInfo->mBlist, index, startFrame, endFrame, flags, animSpeed, currentTime,
			setFrame, blend_time, ghlInfo->aHeader->numFrames);
	}
	return qfalse;
}
	...
	if (ghoul2.size() > model_index)
	{
		...
		if (res)
		{
			...
			return G2_Set_Bone_Anim(ghlInfo, ghlInfo->mBlist, boneName, startFrame, endFrame, flags, animSpeed,
				currentTime, setFrame, blend_time);
		}
	}
	return qfalse;
}

qboolean G2API_GetBoneAnim(CGhoul2Info_v& ghoul2, const int model_index, const char* boneName, const int currentTime,
	float* currentFrame,
	int* startFrame, int* endFrame, int* flags, float* animSpeed, int* modelList)
{
	...
	if (G2_SetupModelPointers(ghlInfo))
	{
		...
		const qboolean ret = G2_Get_Bone_Anim(ghlInfo, ghlInfo->mBlist, boneName, aCurrentTime, currentFrame,
	...
	qhandle_t* modelList, const int blend_time, const int currentTime)
{
	...
	if (res)
	{
		// ensure we flush the cache
		ghlInfo->mSkelFrameNum = 0;
		return G2_Set_Bone_Angles_Index(ghlInfo->mBlist, index, angles, flags, yaw, pitch, roll, modelList,
			ghlInfo->mModelindex, blend_time, currentTime);
	}
	return qfalse;
}
	...
	qhandle_t* modelList, const int blend_time, const int currentTime)
{
	if (ghoul2.size() > model_index)
	{
		...
		if (res)
		{
			// ensure we flush the cache
			ghlInfo->mSkelFrameNum = 0;
			return G2_Set_Bone_Angles(ghlInfo, ghlInfo->mBlist, boneName, angles, flags, up, left, forward, modelList,
				ghlInfo->mModelindex, blend_time, currentTime);
		}
	}
	return qfalse;
...
qboolean G2API_SetBoneAnglesMatrixIndex(CGhoul2Info* ghlInfo, const int index, const mdxaBone_t& matrix,
	const int flags, qhandle_t* modelList, const int blend_time, const int currentTime)
{
	if (G2_SetupModelPointers(ghlInfo))
	{
		// ensure we flush the cache
		ghlInfo->mSkelFrameNum = 0;
		return G2_Set_Bone_Angles_Matrix_Index(ghlInfo->mBlist, index, matrix, flags, modelList, ghlInfo->mModelindex,
			blend_time, currentTime);
	}
	return qfalse;
}
	...
	const int flags, qhandle_t* modelList, const int blend_time, const int currentTime)
{
	if (G2_SetupModelPointers(ghlInfo))
	{
		// ensure we flush the cache
		ghlInfo->mSkelFrameNum = 0;
		return G2_Set_Bone_Angles_Matrix(ghlInfo->mFileName, ghlInfo->mBlist, boneName, matrix, flags, modelList,
			ghlInfo->mModelindex, blend_time, currentTime);
	}
	return qfalse;
}
