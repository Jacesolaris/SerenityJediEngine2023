		...
		if (cg.time - cent->muzzleFlashTime <= MUZZLE_FLASH_TIME + 10)
		{
			...
			if (cent->currentState.eFlags & EF_ALT_FIRING)
			{
				...
				if (weapon->altMuzzleEffect)
				{
					if (!thirdPerson)
					{
						trap->FX_PlayEntityEffectID(weapon->altMuzzleEffect, flashorigin, ref_entity_s.axis, -1, -1, -1, -1);
					}
					else
					{
...
void CG_AddViewWeapon(playerState_t* ps)
{
	...
	CG_AddPlayerWeapon(&hand, ps, &cg_entities[cg.predictedPlayerState.clientNum], ps->persistant[PERS_TEAM], angles,
		qfalse);
	...
	if (ps->eFlags & EF_DUAL_WEAPONS && ps->weapon == WP_BRYAR_PISTOL)
	{
		...
		angles[2] += 20;
		// add everything onto the hand
		cg_add_player_weaponduals(&hand, ps, &cg_entities[cg.predictedPlayerState.clientNum], ps->persistant[PERS_TEAM],
			angles, qfalse, qtrue);
	}
}
...
void* CG_G2WeaponInstance2(const centity_t* cent, int weapon)
{
	const clientInfo_t* ci;
