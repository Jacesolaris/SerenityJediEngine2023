...
void FX_DrawPortableShield(const centity_t* cent)
{
	...
	const int posWidth = cent->currentState.time2 >> 8 & 255;
	const int negWidth = cent->currentState.time2 & 255;
...
static void CG_General(centity_t* cent)
{
		...
		cent->currentState.weapon == G2_MODEL_PART)
	{
		...
		if (!cent->ghoul2)
		{
			...
			if (clEnt && (!BG_InDeathAnim(clEnt->currentState.torsoAnim) || !BG_InDeathAnim(
				clEnt->pe.torso.animationNumber)))
			{
...
===============
*/
extern void CG_AddSaberBlade(centity_t* cent, centity_t* scent, refEntity_t* saber, int renderfx, int modelIndex,
	int saberNum, int bladeNum, vec3_t origin, vec3_t angles, qboolean fromSaber,
	qboolean dontDraw);
		...
		if (s1->owner != ENTITYNUM_NONE)
		{
			...
			if (SaberOwnInfo->saber[0].model[0])
			{
				...
				while (k < SaberOwnInfo->saber[0].numBlades)
				{
					...
					{
						CG_AddSaberBlade(saberOwn, cent, NULL, 0, 0, 0, k, cent->lerpOrigin, bladeAngles, qtrue,
							qfalse);
					}
