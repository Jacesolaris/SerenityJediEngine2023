...
static QINLINE qboolean CG_Piloting(int vehNum)
{
	...
	const centity_t* veh = &cg_entities[vehNum];
...
====================
*/
extern void bg_vehicle_adjust_b_box_for_orientation(const Vehicle_t* veh, vec3_t origin, vec3_t mins, vec3_t maxs, const int clientNum, const int tracemask, void (*local_trace)(trace_t* results, const vec3_t start, const vec3_t minimum_mins, const vec3_t maximum_maxs, const vec3_t end, int pass_entity_num, int content_mask)); // bg_pmove.c
...
void CG_PredictPlayerState(void)
{
	...
	for (cmdNum = current - CMD_BACKUP + 1; cmdNum <= current; cmdNum++)
	{
		...
		{
			const centity_t* blockOpp = &cg_entities[cg.snap->ps.saberLockEnemy];
