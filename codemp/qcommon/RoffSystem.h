...
private:
	...
	static qboolean IsROFF(byte* file); // Makes sure the file is a valid roff file
	static qboolean InitROFF(byte* file, CROFF* obj); // Handles stashing raw roff data into the roff object
	static qboolean InitROFF2(byte* file, CROFF* obj); // Handles stashing raw roff data into the roff object
	...
	static qboolean ApplyROFF(SROFFEntity* roff_ent,
	                          CROFF* roff); // True = success; False = roff complete

	static void ProcessNote(SROFFEntity* roff_ent, char* note);

	static void SetLerp(trajectory_t* tr,
	                    trType_t, vec3_t origin,
	                    vec3_t delta, int time, int rate);

	static qboolean ClearLerp(SROFFEntity* roff_ent); // Clears out the angular and position lerp fields
...
public:
	...
	qboolean Play(int entID, int roffID, qboolean doTranslation, qboolean isClient);
	...
	static qboolean PurgeEnt(char* file); // Purge the specified entity from the entity list by name
