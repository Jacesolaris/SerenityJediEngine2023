...
qboolean FS_Initialized(void) {
	return static_cast<qboolean>(fs_searchpaths != NULL);
}
	...
	do
	{
		...
		for (const searchpath_t* search = fs_searchpaths; search; search = search->next) {
			...
			if (search->pack && search->pack->hashTable[hash]) {
				...
				// autoexec_mp.cfg and SerenityJediEngine2023-SP.cfg can only be loaded outside of pk3 files.
				if (isUserConfig)
				{
					continue;
				}
					...
					if (!FS_FilenameCompare(pakFile->name, filename)) {
						...
						if (!(pak->referenced & FS_GENERAL_REF)) {
							if (!FS_IsExt(filename, ".shader", l) &&
								...
								Q_stricmp(filename, "SerenityJediEngine2023-jampgamex86.dll") != 0 &&
...
int	FS_GetModList(char* listbuf, int bufsize) {
	...
	*listbuf = 0;
	...
	char** pFiles0 = Sys_ListFiles(fs_homepath->string, NULL, NULL, &dummy, qtrue);
	char** pFiles1 = Sys_ListFiles(fs_basepath->string, NULL, NULL, &dummy, qtrue);
	char** pFiles2 = Sys_ListFiles(fs_cdpath->string, NULL, NULL, &dummy, qtrue);
	...
	for (int i = 0; i < nPotential; i++) {
		...
		if (Q_stricmpn(name, ".", 1)) {
			...
			nPaks = 0;
			char** pPaks = Sys_ListFiles(path, ".pk3", NULL, &nPaks, qfalse);
...
void FS_Startup(const char* gameName) {
	...
	fs_gamedirvar = Cvar_Get("fs_game", "SerenityJediEngine2023", CVAR_INIT | CVAR_SYSTEMINFO, "Mod directory");
...
void FS_FilenameCompletion(const char* dir, const char* ext, qboolean stripExt, callbackFunc_t callback, qboolean allowNonPureFilesOnDisk) {
	...
	char** filenames = FS_ListFilteredFiles(dir, ext, NULL, &nfiles);
			...
			if (file != INVALID_HANDLE_VALUE)
			{
				...
				if (WriteFile(file, data, dataLength, &bytesWritten, NULL))
				{
					...
					if (tempFilePath)
					{
						...
						*tempFilePath = static_cast<char*>(Z_Malloc(fileNameLen + 1, TAG_FILESYS));
