...
static void SV_GetServerinfo(char* buffer, const int bufferSize)
{
	if (bufferSize < 1)
	{
...
static qboolean SV_GetEntityToken(char* buffer, const int bufferSize)
{
	char* s;
...
qboolean SV_ROFF_Play(const int entID, const int roffID, const qboolean doTranslation)
{
                                          ...
                                          const int use_lod, const float fRadius)
{
	re->G2API_CollisionDetectCache(collRecMap, *static_cast<CGhoul2Info_v*>(ghoul2), angles, position, frameNumber,
	                               entNum, rayStart, rayEnd, scale, G2VertSpaceServer, traceFlags, use_lod, fRadius);
}
...
static int SV_G2API_CopyGhoul2Instance(void* g2From, void* g2To, const int model_index)
{
	return re->G2API_CopyGhoul2Instance(*static_cast<CGhoul2Info_v*>(g2From), *static_cast<CGhoul2Info_v*>(g2To),
	                                    model_index);
}

static void SV_G2API_CopySpecificGhoul2Model(void* g2From, const int modelFrom, void* g2To, const int modelTo)
{
	if (!g2From || !g2To)
	{
		return;
...
case G_GET_ENTITY_TOKEN:
	...
	*/
	...
	case G_G2_SETMODELS:
		re->G2API_SetGhoul2ModelIndexes(*(CGhoul2Info_v*)args[1], static_cast<qhandle_t*>(VMA(2)),
		                                static_cast<qhandle_t*>(VMA(3)));
		return 0;

	case G_G2_GETBOLT:
		return re->G2API_GetBoltMatrix(*(CGhoul2Info_v*)args[1], args[2], args[3], static_cast<mdxaBone_t*>(VMA(4)),
	...
	case G_G2_ANGLEOVERRIDE:
		return re->G2API_SetBoneAngles(*(CGhoul2Info_v*)args[1], args[2], static_cast<const char*>(VMA(3)),
	...
	case G_G2_PLAYANIM:
		return re->G2API_SetBoneAnim(*(CGhoul2Info_v*)args[1], args[2], static_cast<const char*>(VMA(3)), args[4],
		                             args[5],
	...
	case G_G2_COLLISIONDETECT:
		re->G2API_CollisionDetect(static_cast<CollisionRecord_t*>(VMA(1)), *(CGhoul2Info_v*)args[2],
	...
	case G_G2_COLLISIONDETECTCACHE:
		re->G2API_CollisionDetectCache(static_cast<CollisionRecord_t*>(VMA(1)), *(CGhoul2Info_v*)args[2],
		                               static_cast<const float*>(VMA(3)), static_cast<const float*>(VMA(4)), args[5],
		                               args[6],
	...
	case G_G2_SETSURFACEONOFF:
		return re->G2API_SetSurfaceOnOff(*(CGhoul2Info_v*)args[1], static_cast<const char*>(VMA(2)),
		                                 /*(const int)VMA(3)*/
		                                 args[3]);
	...
	case G_G2_RAGPCJCONSTRAINT:
		return re->G2API_RagPCJConstraint(*(CGhoul2Info_v*)args[1], static_cast<const char*>(VMA(2)),
	...
	case G_G2_RAGEFFECTORGOAL:
		return re->G2API_RagEffectorGoal(*(CGhoul2Info_v*)args[1], static_cast<const char*>(VMA(2)),
		                                 static_cast<float*>(VMA(3)));
	case G_G2_GETRAGBONEPOS:
		return re->G2API_GetRagBonePos(*(CGhoul2Info_v*)args[1], static_cast<const char*>(VMA(2)),
		                               static_cast<float*>(VMA(3)),
		                               static_cast<float*>(VMA(4)), static_cast<float*>(VMA(5)),
		                               static_cast<float*>(VMA(6)));
	case G_G2_RAGEFFECTORKICK:
		return re->G2API_RagEffectorKick(*(CGhoul2Info_v*)args[1], static_cast<const char*>(VMA(2)),
		                                 static_cast<float*>(VMA(3)));
	...
	case G_G2_SETBONEIKSTATE:
		return re->G2API_SetBoneIKState(*(CGhoul2Info_v*)args[1], args[2], static_cast<const char*>(VMA(3)), args[4],
...
void SV_BindGame(void)
{
	...
	if (gvm && !gvm->isLegacy)
	{
		...
		const auto GetGameAPI = (GetGameAPI_t)gvm->GetModuleAPI;
