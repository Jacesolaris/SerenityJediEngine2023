...
qboolean GVM_NAV_ClearPathToPoint(int entID, vec3_t pmins, vec3_t pmaxs, vec3_t point, int clipmask, int okToHitEnt) {
	if (gvm->isLegacy)
		return static_cast<qboolean>(VM_Call(gvm, GAME_NAV_CLEARPATHTOPOINT, entID, reinterpret_cast<intptr_t>(pmins),
			reinterpret_cast<intptr_t>(pmaxs), reinterpret_cast<intptr_t>(point), clipmask,
			okToHitEnt));
	VMSwap v(gvm);
...
static sharedEntity_t* ConvertedEntity(sharedEntity_t* ent) { //Return an entity with the memory shifted around to allow reading/modifying VM memory
	...
	i = 0;
	gLocalModifier.parms = static_cast<parms_t*>(VM_ArgPtr((intptr_t)ent->parms));
	while (i < NUM_BSETS)
	{
		gLocalModifier.behaviorSet[i] = static_cast<char*>(VM_ArgPtr((intptr_t)ent->behaviorSet[i]));
		i++;
	}
	i = 0;
	gLocalModifier.script_targetname = static_cast<char*>(VM_ArgPtr((intptr_t)ent->script_targetname));
	gLocalModifier.delayScriptTime = ent->delayScriptTime;
	gLocalModifier.fullName = static_cast<char*>(VM_ArgPtr((intptr_t)ent->fullName));
	gLocalModifier.targetname = static_cast<char*>(VM_ArgPtr((intptr_t)ent->targetname));
	gLocalModifier.classname = static_cast<char*>(VM_ArgPtr((intptr_t)ent->classname));
...
static int SV_PrecisionTimerEnd(void* timer) {
	timing_c* theTimer = static_cast<timing_c*>(timer); //this is the pointer we assigned in start, so we can directly cast it back
...
static int SV_BotChooseNBGItem(int goalstate, vec3_t origin, int* inventory, int travelflags, void* ltg, float maxtime) {
	return botlib_export->ai.BotChooseNBGItem(goalstate, origin, inventory, travelflags, static_cast<bot_goal_s*>(ltg), maxtime);
}
...
static void SV_BotMoveToGoal(void* result, int movestate, void* goal, int travelflags) {
	botlib_export->ai.BotMoveToGoal(static_cast<bot_moveresult_s*>(result), movestate, static_cast<bot_goal_s*>(goal), travelflags);
}
...
static int SV_BotMovementViewTarget(int movestate, void* goal, int travelflags, float lookahead, vec3_t target) {
	return botlib_export->ai.BotMovementViewTarget(movestate, static_cast<bot_goal_s*>(goal), travelflags, lookahead, target);
}
...
static int SV_BotPredictVisiblePosition(vec3_t origin, int areanum, void* goal, int travelflags, vec3_t target) {
	return botlib_export->ai.BotPredictVisiblePosition(origin, areanum, static_cast<bot_goal_s*>(goal), travelflags, target);
}
...
static int SV_AAS_PredictClientMovement(void* move, int entnum, vec3_t origin, int presencetype, int onground, vec3_t velocity, vec3_t cmdmove, int cmdframes, int maxframes, float frametime, int stopevent, int stopareanum, int visualize) {
	return botlib_export->aas.AAS_PredictClientMovement(static_cast<aas_clientmove_s*>(move), entnum, origin, presencetype, onground, velocity, cmdmove, cmdframes, maxframes, frametime, stopevent, stopareanum, visualize);
}
...
static int SV_AAS_PredictRoute(void* route, int areanum, vec3_t origin, int goalareanum, int travelflags, int maxareas, int maxtime, int stopevent, int stopcontents, int stoptfl, int stopareanum) {
	return botlib_export->aas.AAS_PredictRoute(static_cast<aas_predictroute_s*>(route), areanum, origin, goalareanum, travelflags, maxareas, maxtime, stopevent, stopcontents, stoptfl, stopareanum);
}
...
static void SV_G2API_SetGhoul2ModelIndexes(void* ghoul2, qhandle_t* modelList, qhandle_t* skinList) {
	re->G2API_SetGhoul2ModelIndexes(*static_cast<CGhoul2Info_v*>(ghoul2), modelList, skinList);
}
...
static qboolean SV_G2API_GetBoltMatrix(void* ghoul2, const int model_index, const int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, const int frameNum, qhandle_t* modelList, vec3_t scale) {
	return re->G2API_GetBoltMatrix(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, bolt_index, matrix, angles, position, frameNum, modelList, scale);
}

static qboolean SV_G2API_GetBoltMatrix_NoReconstruct(void* ghoul2, const int model_index, const int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, const int frameNum, qhandle_t* modelList, vec3_t scale) {
	re->G2API_BoltMatrixReconstruction(qfalse);
	return re->G2API_GetBoltMatrix(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, bolt_index, matrix, angles, position, frameNum, modelList, scale);
}

static qboolean SV_G2API_GetBoltMatrix_NoRecNoRot(void* ghoul2, const int model_index, const int bolt_index, mdxaBone_t* matrix, const vec3_t angles, const vec3_t position, const int frameNum, qhandle_t* modelList, vec3_t scale) {
	re->G2API_BoltMatrixReconstruction(qfalse);
	re->G2API_BoltMatrixSPMethod(qtrue);
	return re->G2API_GetBoltMatrix(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, bolt_index, matrix, angles, position, frameNum, modelList, scale);
}

static int SV_G2API_InitGhoul2Model(void** ghoul2Ptr, const char* fileName, int model_index, qhandle_t customSkin, qhandle_t customShader, int modelFlags, int lodBias) {
#ifdef _FULL_G2_LEAK_CHECKING
	g_G2AllocServer = 1;
#endif
	return re->G2API_InitGhoul2Model((CGhoul2Info_v**)ghoul2Ptr, fileName, model_index, customSkin, customShader, modelFlags, lodBias);
}

static qboolean SV_G2API_SetSkin(void* ghoul2, int model_index, qhandle_t customSkin, qhandle_t renderSkin) {
...
static void SV_G2API_CollisionDetect(CollisionRecord_t* collRecMap, void* ghoul2, const vec3_t angles, const vec3_t position, int frameNumber, int entNum, vec3_t rayStart, vec3_t rayEnd, vec3_t scale, int traceFlags, int useLod, float fRadius) {
	re->G2API_CollisionDetect(collRecMap, *static_cast<CGhoul2Info_v*>(ghoul2), angles, position, frameNumber, entNum, rayStart, rayEnd, scale, G2VertSpaceServer, traceFlags, useLod, fRadius);
}
...
static void SV_G2API_CollisionDetectCache(CollisionRecord_t* collRecMap, void* ghoul2, const vec3_t angles, const vec3_t position, int frameNumber, int entNum, vec3_t rayStart, vec3_t rayEnd, vec3_t scale, int traceFlags, int useLod, float fRadius) {
	re->G2API_CollisionDetectCache(collRecMap, *static_cast<CGhoul2Info_v*>(ghoul2), angles, position, frameNumber, entNum, rayStart, rayEnd, scale, G2VertSpaceServer, traceFlags, useLod, fRadius);
}
...
static qboolean SV_G2API_SetBoneAngles(void* ghoul2, int model_index, const char* boneName, const vec3_t angles, const int flags, const int up, const int right, const int forward, qhandle_t* modelList, int blendTime, int currentTime) {
	return re->G2API_SetBoneAngles(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, boneName, angles, flags, static_cast<const Eorientations>(up), static_cast<const Eorientations>(right), static_cast<const Eorientations>(forward), modelList, blendTime, currentTime);
}

static qboolean SV_G2API_SetBoneAnim(void* ghoul2, const int model_index, const char* boneName, const int startFrame, const int endFrame, const int flags, const float animSpeed, const int currentTime, const float setFrame, const int blendTime) {
	return re->G2API_SetBoneAnim(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, boneName, startFrame, endFrame, flags, animSpeed, currentTime, setFrame, blendTime);
}

static qboolean SV_G2API_GetBoneAnim(void* ghoul2, const char* boneName, const int currentTime, float* currentFrame, int* startFrame, int* endFrame, int* flags, float* animSpeed, int* modelList, const int model_index) {
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
	return re->G2API_GetBoneAnim(g2, model_index, boneName, currentTime, currentFrame, startFrame, endFrame, flags, animSpeed, modelList);
}

static void SV_G2API_GetGLAName(void* ghoul2, int model_index, char* fillBuf) {
...
static int SV_G2API_CopyGhoul2Instance(void* g2From, void* g2To, int model_index) {
	return re->G2API_CopyGhoul2Instance(*static_cast<CGhoul2Info_v*>(g2From), *static_cast<CGhoul2Info_v*>(g2To), model_index);
}
...
static void SV_G2API_CopySpecificGhoul2Model(void* g2From, int modelFrom, void* g2To, int modelTo)
{
	...
	re->G2API_CopySpecificG2Model(*static_cast<CGhoul2Info_v*>(g2From), modelFrom, *static_cast<CGhoul2Info_v*>(g2To), modelTo);
}
...
static void SV_G2API_DuplicateGhoul2Instance(void* g2From, void** g2To) {
	...
	g_G2AllocServer = 1;
#endif
	re->G2API_DuplicateGhoul2Instance(*static_cast<CGhoul2Info_v*>(g2From), (CGhoul2Info_v**)g2To);
}

static qboolean SV_G2API_HasGhoul2ModelOnIndex(void* ghlInfo, int model_index) {
	return re->G2API_HasGhoul2ModelOnIndex(static_cast<CGhoul2Info_v**>(ghlInfo), model_index);
}

static qboolean SV_G2API_RemoveGhoul2Model(void* ghlInfo, int model_index) {
#ifdef _FULL_G2_LEAK_CHECKING
	g_G2AllocServer = 1;
#endif
...
static int SV_G2API_AddBolt(void* ghoul2, int model_index, const char* boneName) {
	return re->G2API_AddBolt(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, boneName);
}

static void SV_G2API_SetBoltInfo(void* ghoul2, int model_index, int boltInfo) {
	re->G2API_SetBoltInfo(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, boltInfo);
}

static qboolean SV_G2API_SetRootSurface(void* ghoul2, const int model_index, const char* surfaceName) {
	return re->G2API_SetRootSurface(*static_cast<CGhoul2Info_v*>(ghoul2), model_index, surfaceName);
}
...
static qboolean SV_G2API_DoesBoneExist(void* ghoul2, int model_index, const char* boneName) {
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
	return re->G2API_DoesBoneExist(g2, model_index, boneName);
}

static int SV_G2API_GetSurfaceRenderStatus(void* ghoul2, const int model_index, const char* surfaceName) {
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
...
static qboolean SV_G2API_RemoveBone(void* ghoul2, const char* boneName, int model_index) {
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
...
static void SV_G2API_GetSurfaceName(void* ghoul2, int surfNumber, int model_index, char* fillBuf) {
	CGhoul2Info_v& g2 = *static_cast<CGhoul2Info_v*>(ghoul2);
...
static qboolean CL_SEP_GetStringTextString(const char* text, char* buffer, int bufferLength)
{
	assert(text && buffer);
...
intptr_t SV_GameSystemCalls(intptr_t* args) {
	...
	case TRAP_ANGLEVECTORS:
		AngleVectors(static_cast<const float*>(VMA(1)), static_cast<float*>(VMA(2)), static_cast<float*>(VMA(3)), static_cast<float*>(VMA(4)));
		return 0;
	...
	case G_LOCATE_GAME_DATA:
		SV_LocateGameData(static_cast<sharedEntity_t*>(VMA(1)), args[2], args[3], static_cast<playerState_s*>(VMA(4)), args[5]);
		return 0;
	...
	case G_ENTITIES_IN_BOX:
		return SV_AreaEntities(static_cast<const float*>(VMA(1)), static_cast<const float*>(VMA(2)), static_cast<int*>(VMA(3)), args[4]);

	case G_ENTITY_CONTACT:
		return SV_EntityContact(static_cast<const float*>(VMA(1)), static_cast<const float*>(VMA(2)), static_cast<const sharedEntity_t*>(VMA(3)), /*int capsule*/ qfalse);

	case G_ENTITY_CONTACTCAPSULE:
		return SV_EntityContact(static_cast<const float*>(VMA(1)), static_cast<const float*>(VMA(2)), static_cast<const sharedEntity_t*>(VMA(3)), /*int capsule*/ qtrue);

	case G_TRACE:
		SV_Trace(static_cast<trace_t*>(VMA(1)), static_cast<const float*>(VMA(2)), static_cast<const float*>(VMA(3)), static_cast<const float*>(VMA(4)), static_cast<const float*>(VMA(5)), args[6], args[7], /*int capsule*/ qfalse, /*args[8]*/0, args[9]);
		return 0;
	case G_G2TRACE:
		SV_Trace(static_cast<trace_t*>(VMA(1)), static_cast<const float*>(VMA(2)), static_cast<const float*>(VMA(3)), static_cast<const float*>(VMA(4)), static_cast<const float*>(VMA(5)), args[6], args[7], /*int capsule*/ qfalse, args[8], args[9]);
		return 0;
	case G_TRACECAPSULE:
		SV_Trace(static_cast<trace_t*>(VMA(1)), static_cast<const float*>(VMA(2)), static_cast<const float*>(VMA(3)), static_cast<const float*>(VMA(4)), static_cast<const float*>(VMA(5)), args[6], args[7], /*int capsule*/ qtrue, args[8], args[9]);
		return 0;
	...
	case G_ICARUS_RUNSCRIPT:
		return ICARUS_RunScript(ConvertedEntity(static_cast<sharedEntity_t*>(VMA(1))), static_cast<const char*>(VMA(2)));
	...
	case G_NAV_GETBESTPATHBETWEENENTS:
		return navigator.GetBestPathBetweenEnts(static_cast<sharedEntity_t*>(VMA(1)), static_cast<sharedEntity_t*>(VMA(2)), args[3]);
	...
	case BOTLIB_TEST:
		return botlib_export->Test(args[1], static_cast<char*>(VMA(2)), static_cast<float*>(VMA(3)), static_cast<float*>(VMA(4)));
	...
	case BOTLIB_AAS_BBOX_AREAS:
		return botlib_export->aas.AAS_BBoxAreas(static_cast<float*>(VMA(1)), static_cast<float*>(VMA(2)), static_cast<int*>(VMA(3)), args[4]);
	case BOTLIB_AAS_AREA_INFO:
		return botlib_export->aas.AAS_AreaInfo(args[1], static_cast<aas_areainfo_s*>(VMA(2)));
	case BOTLIB_AAS_ALTERNATIVE_ROUTE_GOAL:
		return botlib_export->aas.AAS_AlternativeRouteGoals(static_cast<float*>(VMA(1)), args[2], static_cast<float*>(VMA(3)), args[4], args[5], static_cast<aas_altroutegoal_s*>(VMA(6)), args[7], args[8]);
	...
	case BOTLIB_AAS_PRESENCE_TYPE_BOUNDING_BOX:
		botlib_export->aas.AAS_PresenceTypeBoundingBox(args[1], static_cast<float*>(VMA(2)), static_cast<float*>(VMA(3)));
		return 0;
	...
	case BOTLIB_AAS_TRACE_AREAS:
		return botlib_export->aas.AAS_TraceAreas(static_cast<float*>(VMA(1)), static_cast<float*>(VMA(2)), static_cast<int*>(VMA(3)), static_cast<float(*)[3]>(VMA(4)), args[5]);
	...
	case BOTLIB_AAS_VECTOR_FOR_BSP_EPAIR_KEY:
		return botlib_export->aas.AAS_VectorForBSPEpairKey(args[1], static_cast<char*>(VMA(2)), static_cast<float*>(VMA(3)));
	...
	case BOTLIB_AAS_PREDICT_ROUTE:
		return botlib_export->aas.AAS_PredictRoute(static_cast<aas_predictroute_s*>(VMA(1)), args[2], static_cast<float*>(VMA(3)), args[4], args[5], args[6], args[7], args[8], args[9], args[10], args[11]);

	case BOTLIB_AAS_SWIMMING:
		return botlib_export->aas.AAS_Swimming(static_cast<float*>(VMA(1)));
	case BOTLIB_AAS_PREDICT_CLIENT_MOVEMENT:
		return botlib_export->aas.AAS_PredictClientMovement(static_cast<aas_clientmove_s*>(VMA(1)), args[2], static_cast<float*>(VMA(3)), args[4], args[5],
			static_cast<float*>(VMA(6)), static_cast<float*>(VMA(7)), args[8], args[9], VMF(10), args[11], args[12], args[13]);
	...
	case BOTLIB_AI_CHOOSE_LTG_ITEM:
		return botlib_export->ai.BotChooseLTGItem(args[1], static_cast<float*>(VMA(2)), static_cast<int*>(VMA(3)), args[4]);
	case BOTLIB_AI_CHOOSE_NBG_ITEM:
		return botlib_export->ai.BotChooseNBGItem(args[1], static_cast<float*>(VMA(2)), static_cast<int*>(VMA(3)), args[4], static_cast<bot_goal_s*>(VMA(5)), VMF(6));
	case BOTLIB_AI_TOUCHING_GOAL:
		return botlib_export->ai.BotTouchingGoal(static_cast<float*>(VMA(1)), static_cast<bot_goal_s*>(VMA(2)));
	case BOTLIB_AI_ITEM_GOAL_IN_VIS_BUT_NOT_VISIBLE:
		return botlib_export->ai.BotItemGoalInVisButNotVisible(args[1], static_cast<float*>(VMA(2)), static_cast<float*>(VMA(3)), static_cast<bot_goal_s*>(VMA(4)));
	...
	case BOTLIB_AI_MOVEMENT_VIEW_TARGET:
		return botlib_export->ai.BotMovementViewTarget(args[1], static_cast<bot_goal_s*>(VMA(2)), args[3], VMF(4), static_cast<float*>(VMA(5)));
	case BOTLIB_AI_PREDICT_VISIBLE_POSITION:
		return botlib_export->ai.BotPredictVisiblePosition(static_cast<float*>(VMA(1)), args[2], static_cast<bot_goal_s*>(VMA(3)), args[4], static_cast<float*>(VMA(5)));
	...
	case G_G2_SETMODELS:
		re->G2API_SetGhoul2ModelIndexes(*(CGhoul2Info_v*)args[1], static_cast<qhandle_t*>(VMA(2)), static_cast<qhandle_t*>(VMA(3)));
		return 0;

	case G_G2_GETBOLT:
		return re->G2API_GetBoltMatrix(*(CGhoul2Info_v*)args[1], args[2], args[3], static_cast<mdxaBone_t*>(VMA(4)), static_cast<const float*>(VMA(5)), static_cast<const float*>(VMA(6)), args[7], static_cast<qhandle_t*>(VMA(8)), static_cast<float*>(VMA(9)));
	...
	case G_G2_ADDBOLT:
		return	re->G2API_AddBolt(*(CGhoul2Info_v*)args[1], args[2], static_cast<const char*>(VMA(3)));
	...
	case G_G2_ANGLEOVERRIDE:
		return re->G2API_SetBoneAngles(*(CGhoul2Info_v*)args[1], args[2], static_cast<const char*>(VMA(3)), static_cast<float*>(VMA(4)), args[5],
			static_cast<const Eorientations>(args[6]), static_cast<const Eorientations>(args[7]), static_cast<const Eorientations>(args[8]),
			static_cast<qhandle_t*>(VMA(9)), args[10], args[11]);

	case G_G2_PLAYANIM:
		return re->G2API_SetBoneAnim(*(CGhoul2Info_v*)args[1], args[2], static_cast<const char*>(VMA(3)), args[4], args[5],
	...
	case G_G2_GETBONEANIM:
	{
		...
		return re->G2API_GetBoneAnim(g2, model_index, static_cast<const char*>(VMA(2)), args[3], static_cast<float*>(VMA(4)), static_cast<int*>(VMA(5)),
			static_cast<int*>(VMA(6)), static_cast<int*>(VMA(7)), static_cast<float*>(VMA(8)), static_cast<int*>(VMA(9)));
	}
	...
	{ //Since returning a pointer in such a way to a VM seems to cause MASSIVE FAILURE<tm>, we will shove data into the pointer the vm passes instead
		char* point = static_cast<char*>(VMA(3));
	...
	case G_G2_DUPLICATEGHOUL2INSTANCE:
		...
		g_G2AllocServer = 1;
#endif
		re->G2API_DuplicateGhoul2Instance(*(CGhoul2Info_v*)args[1], static_cast<CGhoul2Info_v**>(VMA(2)));
		return 0;
	...
	case G_G2_COLLISIONDETECT:
		re->G2API_CollisionDetect(static_cast<CollisionRecord_t*>(VMA(1)), *(CGhoul2Info_v*)args[2], static_cast<const float*>(VMA(3)), static_cast<const float*>(VMA(4)), args[5], args[6], static_cast<float*>(VMA(7)), static_cast<float*>(VMA(8)), static_cast<float*>(VMA(9)), G2VertSpaceServer, args[10], args[11], VMF(12));
		return 0;

	case G_G2_COLLISIONDETECTCACHE:
		re->G2API_CollisionDetectCache(static_cast<CollisionRecord_t*>(VMA(1)), *(CGhoul2Info_v*)args[2], static_cast<const float*>(VMA(3)), static_cast<const float*>(VMA(4)), args[5], args[6], static_cast<float*>(VMA(7)), static_cast<float*>(VMA(8)), static_cast<float*>(VMA(9)), G2VertSpaceServer, args[10], args[11], VMF(12));
		return 0;

	case G_G2_SETROOTSURFACE:
		return re->G2API_SetRootSurface(*(CGhoul2Info_v*)args[1], args[2], static_cast<const char*>(VMA(3)));

	case G_G2_SETSURFACEONOFF:
		return re->G2API_SetSurfaceOnOff(*(CGhoul2Info_v*)args[1], static_cast<const char*>(VMA(2)), /*(const int)VMA(3)*/args[3]);
	...
	case G_G2_SETRAGDOLL:
	{
		...
		sharedRagDollParams_t* rdParamst = static_cast<sharedRagDollParams_t*>(VMA(2));
		CRagDollParams rdParams;
	...
	case G_G2_RAGPCJCONSTRAINT:
		return re->G2API_RagPCJConstraint(*(CGhoul2Info_v*)args[1], static_cast<const char*>(VMA(2)), static_cast<float*>(VMA(3)), static_cast<float*>(VMA(4)));
	case G_G2_RAGPCJGRADIENTSPEED:
		return re->G2API_RagPCJGradientSpeed(*(CGhoul2Info_v*)args[1], static_cast<const char*>(VMA(2)), VMF(3));
	case G_G2_RAGEFFECTORGOAL:
		return re->G2API_RagEffectorGoal(*(CGhoul2Info_v*)args[1], static_cast<const char*>(VMA(2)), static_cast<float*>(VMA(3)));
	case G_G2_GETRAGBONEPOS:
		return re->G2API_GetRagBonePos(*(CGhoul2Info_v*)args[1], static_cast<const char*>(VMA(2)), static_cast<float*>(VMA(3)), static_cast<float*>(VMA(4)), static_cast<float*>(VMA(5)), static_cast<float*>(VMA(6)));
	case G_G2_RAGEFFECTORKICK:
		return re->G2API_RagEffectorKick(*(CGhoul2Info_v*)args[1], static_cast<const char*>(VMA(2)), static_cast<float*>(VMA(3)));
	case G_G2_RAGFORCESOLVE:
		return re->G2API_RagForceSolve(*(CGhoul2Info_v*)args[1], static_cast<qboolean>(args[2]));

	case G_G2_SETBONEIKSTATE:
		return re->G2API_SetBoneIKState(*(CGhoul2Info_v*)args[1], args[2], static_cast<const char*>(VMA(3)), args[4], static_cast<sharedSetBoneIKStateParams_t*>(VMA(5)));
	case G_G2_IKMOVE:
		return re->G2API_IKMove(*(CGhoul2Info_v*)args[1], args[2], static_cast<sharedIKMoveParams_t*>(VMA(3)));
	...
	{
		re->G2API_AttachInstanceToEntNum(*(CGhoul2Info_v*)args[1], args[2], static_cast<qboolean>(args[3]));
	}
	return 0;
	...
	{ //Since returning a pointer in such a way to a VM seems to cause MASSIVE FAILURE<tm>, we will shove data into the pointer the vm passes instead
		char* point = static_cast<char*>(VMA(4));
...
void SV_BindGame(void) {
	...
	char				dllName[MAX_OSPATH] = "SerenityJediEngine2023-jampgame" ARCH_STRING DLL_EXT;
	...
	if (gvm && !gvm->isLegacy) {
		...
		const GetGameAPI_t GetGameAPI = (GetGameAPI_t)gvm->GetModuleAPI;
