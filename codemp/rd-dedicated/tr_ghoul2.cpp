...
class CBoneCache
{
	...
	void SmoothLow(int index)
	{
		...
		const auto skel = (mdxaSkel_t*)((byte*)header + sizeof(mdxaHeader_t) + offsets->offsets[index]);
...
public:
	                                                               ...
	                                                               incomingTime(0), mCurrentTouchRender(0)
	{
		...
		{
			const mdxaSkel_t* skel = (mdxaSkel_t*)((byte*)header + sizeof(mdxaHeader_t) + offsets->offsets[i]);
			//mSkels[i]=skel;
			//ditto
...
int G2_GetBoneDependents(CGhoul2Info& ghoul2, const int boneNum, int* tempDependents, int maxDep)
{
	...
	const mdxaSkel_t* skel = (mdxaSkel_t*)((byte*)boneCache.header + sizeof(mdxaHeader_t) + offsets->offsets[boneNum]);
	int i;
	int ret = 0;
...
void G2_RagGetBoneBasePoseMatrixLow(const CGhoul2Info& ghoul2, const int boneNum, mdxaBone_t& boneMatrix,
...
void G2_TimingModel(boneInfo_t& bone, const int current_time, const int numFramesInFile, int& current_frame,
                    int& newFrame,
                    float& lerp)
{
...
void G2_TransformBone(const int child, CBoneCache& BC)
{
	...
	if (boneListIndex != -1)
	{
		...
		{
			float blend_time = BC.incomingTime - boneList[boneListIndex].blendStart;
	...
	const int parent = BC.mFinalBones[child].parent;
	assert(parent == -1 && child == 0 || parent >= 0 && parent < static_cast<int>(BC.mBones.size()));
	if (angleOverride & BONE_ANGLES_REPLACE)
	{
		...
		if (isRag)
		{
			...
			float blend_time = BC.incomingTime - boneList[boneListIndex].boneBlendStart;
		...
		else
		{
			...
			if (boneOverride.boneBlendTime && boneOverride.boneBlendTime + boneOverride.boneBlendStart < BC.
				incomingTime)
			{
	...
	else if (angleOverride & BONE_ANGLES_PREMULT)
	{
		...
		else
		{
			if (!child)
			{
				...
				if (HackadelicOnClient)
				{
					Multiply_3x4Matrix(&BC.mFinalBones[child].boneMatrix, &BC.rootMatrix,
			...
			else
			{
				...
				if (HackadelicOnClient)
				{
					Multiply_3x4Matrix(&BC.mFinalBones[child].boneMatrix, &BC.mFinalBones[parent].boneMatrix,
					                   &boneList[boneListIndex].newMatrix);
				}
				else
				{
...
void G2_ProcessSurfaceBolt(mdxaBone_v& bonePtr, mdxmSurface_t* surface, const int boltNum, boltInfo_v& boltList,
...
void ProcessModelBoltSurfaces(const int surfaceNum, surfaceInfo_v& rootSList,
                              mdxaBone_v& bonePtr, model_t* currentModel, const int lod, boltInfo_v& boltList)
{
#ifdef G2_PERFORMANCE_ANALYSIS
