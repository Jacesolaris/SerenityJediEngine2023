...
int trap_CM_LerpTag(orientation_t* tag, clipHandle_t mod, int start_frame, int end_frame, float frac, const char* tagName) {
...
qboolean trap_G2API_SetBoneAngles(void* ghoul2, int model_index, const char* boneName, const vec3_t angles, const int flags, const int up, const int right, const int forward, qhandle_t* modelList, int blend_time, int current_time) {
	return Q_syscall(UI_G2_ANGLEOVERRIDE, ghoul2, model_index, boneName, angles, flags, up, right, forward, modelList, blend_time, current_time);
}
...
qboolean trap_G2API_GetBoneAnim(void* ghoul2, const char* boneName, const int current_time, float* current_frame, int* start_frame, int* end_frame, int* flags, float* animSpeed, int* modelList, const int model_index) {
	return Q_syscall(UI_G2_GETBONEANIM, ghoul2, boneName, current_time, current_frame, start_frame, end_frame, flags, animSpeed, modelList, model_index);
}
qboolean trap_G2API_GetBoneFrame(void* ghoul2, const char* boneName, const int current_time, float* current_frame, int* modelList, const int model_index) {
	return Q_syscall(UI_G2_GETBONEFRAME, ghoul2, boneName, current_time, current_frame, modelList, model_index);
}
