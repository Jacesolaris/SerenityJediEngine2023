	...
	do
	{
		...
		for (const searchpath_t* search = fs_searchpaths; search; search = search->next) {
			...
			if (search->pack && search->pack->hashTable[hash]) {
				// autoexec_sp.cfg and SerenityJediEngine2023-SP.cfg can only be loaded outside of pk3 files.
				if (isUserConfig) {
					continue;
				}
...
void FS_Startup(const char* gameName) {
	...
	fs_gamedirvar = Cvar_Get("fs_game", "SerenityJediEngine2023", CVAR_INIT | CVAR_SYSTEMINFO);
