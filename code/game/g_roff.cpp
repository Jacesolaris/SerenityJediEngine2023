...
static qboolean G_ValidRoff(roff_hdr2_t* header)
{
	if (strncmp(header->mHeader, "ROFF", 4) == 0)
	{
		...
		if (LittleLong header->mVersion == ROFF_VERSION && LittleFloat((roff_hdr_t*)header)->mCount > 0.0f)
		{
...
{
	const auto header = (roff_hdr_t*)data;
	int count;
	int i;
	...
	if (LittleLong header->mVersion == ROFF_VERSION)
	{
		...
		if (mem)
		{
			...
			auto roff_data = (move_rotate_t*)&header[1];
	...
	else if (LittleLong header->mVersion == ROFF_VERSION2)
	{
		...
		const auto hdr = (roff_hdr2_t*)data;
		...
		if (mem)
		{
			...
			// Step past the header to get to the goods
			auto roff_data = (move_rotate2_t*)&hdr[1];
...
int G_LoadRoff(const char* fileName)
{
	...
	const auto header = (roff_hdr2_t*)data;
