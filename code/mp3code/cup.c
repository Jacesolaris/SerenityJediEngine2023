...
// only written into by decode init funcs, then copied to stack struct higher up
...
static long load(const int n)
{
	...
	bits -= n;
	const unsigned long x = bitbuf >> bits;
	bitbuf -= x << bits;
	return x;
}
...
static void unpack_ba()
{
	...
	int nstereo = pMP3Stream->stereo_sb;
	int k = 0;
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < pMP3Stream->nbat[i]; j++, k++)
		{
			mac_load_check(4);
