...
void antialias(float x[], const int n)
{
	for (int k = 0; k < n; k++)
	{
		for (int i = 0; i < 8; i++)
		{
			const float a = x[17 - i];
			const float b = x[18 + i];
...
void ms_process(float x[][1152], const int n) /* sum-difference stereo */
{
	...
	for (int i = 0; i < n; i++)
	{
		const float xl = x[0][i] + x[1][i];
		const float xr = x[0][i] - x[1][i];
		x[0][i] = xl;
		x[1][i] = xr;
	}
}
                      ...
                      const int nsamp, const int ms_mode)
{
	...
	for (cb = cb0; cb < 21; cb++)
	{
		...
		const float fl = lr[ms_mode][isf][0];
		const float fr = lr[ms_mode][isf][1];
		for (j = 0; j < n; j++, i++)
		{
			if (--m < 0)
				goto exit;
