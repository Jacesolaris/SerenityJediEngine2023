...
int head_info2(unsigned char* buf, unsigned int n, MPEG_HEAD* h, int* br)
{
	...
	*br = 0;
	/*-- assume fail --*/
	const int framebytes = head_info(buf, n, h);
...
static int find_sync(unsigned char* buf, int n)
{
	...
	while (i0 < 2000)
	{
		int isync = sync_scan(buf, n, i0);
		i0 = isync + 1;
		isync -= pad;
		if (isync <= 0)
			return 0;
		const int nmatch = sync_test(buf, n, isync, padbytes);
		if (nmatch > 0)
			return isync;
	}
...
static int sync_test(unsigned char* buf, int n, int isync, int padbytes)
{
	...
	for (int i = 0;;)
	{
		const int pad = padbytes * ((buf[i + 2] & 0x02) >> 1);
		i += pad + isync;
		if (i > n)
			break;
		if (!compare(buf, buf + i))
