...
===============
*/
void SV_GetServerinfo(char* buffer, const int bufferSize)
{
	if (bufferSize < 1)
	{
...
qboolean SV_GetEntityToken(char* buffer, const int bufferSize)
{
	char* s;
...
static int SV_G2API_AddBoltSurfNum(CGhoul2Info* ghl_info, const int surfIndex)
{
	return re.G2API_AddBoltSurfNum(ghl_info, surfIndex);
}
...
{
	re.G2API_CopyGhoul2Instance(ghoul2From, ghoul2To, model_index);
}
...
static qboolean SV_G2API_GetBoneAnim(
	...
	float* current_frame, int* start_frame, int* end_frame, int* flags, float* animSpeed, int* modelList)
{
	return re.G2API_GetBoneAnim(ghl_info, boneName, AcurrentTime, current_frame,
	                            start_frame, end_frame, flags, animSpeed, modelList);
}
...
static int SV_G2API_GetSurfaceIndex(CGhoul2Info* ghl_info, const char* surfaceName)
{
	return re.G2API_GetSurfaceIndex(ghl_info, surfaceName);
}
...
static int SV_G2API_GetSurfaceRenderStatus(CGhoul2Info* ghl_info, const char* surfaceName)
{
	return re.G2API_GetSurfaceRenderStatus(ghl_info, surfaceName);
}
...
{
	return re.G2API_SetBoneAnglesOffset(ghl_info, boneName, angles, flags, up, right, forward, modelList, blend_time,
	                                    current_time, offset);
}
...
{
	return re.G2API_SetBoneAnglesMatrix(ghl_info, boneName, matrix, flags, modelList, blend_time, AcurrentTime);
}
...
static qboolean SV_G2API_SetBoneAnglesMatrixIndex(CGhoul2Info* ghl_info, const int index, const mdxaBone_t& matrix,
                                                  ...
                                                  const int AcurrentTime)
{
	return re.G2API_SetBoneAnglesMatrixIndex(ghl_info, index, matrix, flags, modelList, blandeTime, AcurrentTime);
}
...
static qboolean SV_G2API_SetRootSurface(CGhoul2Info_v& ghl_info, const int model_index, const char* surfaceName)
{
	return re.G2API_SetRootSurface(ghl_info, model_index, surfaceName);
}
...
static qboolean SV_G2API_SetSurfaceOnOff(CGhoul2Info* ghl_info, const char* surfaceName, const int flags)
{
	return re.G2API_SetSurfaceOnOff(ghl_info, surfaceName, flags);
}
...
#ifdef JK2_MODE
	const char* gamename = "SerenityJediEngine2023-JKO-game";
#else
	auto gamename = "SerenityJediEngine2023-game";
#endif
