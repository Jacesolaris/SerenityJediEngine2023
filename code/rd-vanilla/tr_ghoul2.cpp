...
class CBoneCache
{
	...
	void SmoothLow(const int index) const
	{
		...
		const mdxaSkel_t* skel = (mdxaSkel_t*)((byte*)header + sizeof(mdxaHeader_t) + offsets->offsets[index]);
	...
	const surfaceInfo_t* surfInfo, const model_t* mod, mdxaBone_t& retMatrix)
{
	...
	if (surfInfo && surfInfo->offFlags == G2SURFACEFLAG_GENERATED)
	{
		...
		VectorClear(pTri[1]);
		VectorClear(pTri[2]);
		const int* piBoneReferences = (int*)((byte*)originalSurf + originalSurf->ofsBoneReferences);
...
qboolean R_LoadMDXM(model_t* mod, void* buffer, const char* mod_name, qboolean& bAlreadyCached)
{
	...
	for (i = 0; i < mdxm->numSurfaces; i++)
	{
		...
		if (!sh)
		{
			surfInfo = (mdxmSurfHierarchy_t*)((byte*)surfInfo + (intptr_t) & static_cast<mdxmSurfHierarchy_t*>(nullptr)->
