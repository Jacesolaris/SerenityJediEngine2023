	...
	for (int l = 0; l < backEnd.refdef.num_dlights; l++)
	{
		...
		for (i = 0; i < tess.numIndexes; i += 3)
		{
			...
			*/
			...
			const auto* ba = reinterpret_cast<byteAlias_t*>(&colorTemp);
...
static void RB_FogPass(void) {
	...
	for (int i = 0; i < tess.numVertexes; i++) {
		auto* ba = (byteAlias_t*)&tess.svars.colors[i];
		ba->i = fog->colorInt;
	}
...
static void RB_IterateStagesGeneric(shaderCommands_t* input)
{
	...
	for (int stage = 0; stage < input->shader->numUnfoggedPasses; stage++)
	{
		...
		alphaGen_t	forceAlphaGen = static_cast<alphaGen_t>(0);
		colorGen_t	forceRGBGen = static_cast<colorGen_t>(0);
