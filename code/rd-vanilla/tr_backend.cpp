...
const void* RB_SetColor(const void* data) {
	const setColorCommand_t* cmd = static_cast<const setColorCommand_t*>(data);
...
const void* RB_StretchPic(const void* data) {
	const stretchPicCommand_t* cmd = static_cast<const stretchPicCommand_t*>(data);
...
{
	const rotatePicCommand_t* cmd = static_cast<const rotatePicCommand_t*>(data);
	...
	byteAlias_t* baDest = reinterpret_cast<byteAlias_t*>(&tess.vertexColors[numVerts + 0]); baDest->ui = baSource->ui;
...
{
	const scissorCommand_t* cmd = static_cast<const scissorCommand_t*>(data);
...
const void* RB_SwapBuffers(const void* data) {
	...
	const swapBuffersCommand_t* cmd = static_cast<const swapBuffersCommand_t*>(data);
	...
	if (r_measureOverdraw->integer) {
		...
		unsigned char* stencilReadback = static_cast<unsigned char*>(R_Malloc(glConfig.vidWidth * glConfig.vidHeight,
...
{
	const setModeCommand_t* cmd = static_cast<const setModeCommand_t*>(data);
