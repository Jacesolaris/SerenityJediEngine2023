             ...
             const int nparams, const png_const_charp units, const png_charpp params)
{
	...
	png_size_t length = strlen(purpose) + 1;
             ...
             const png_const_bytep profile, const png_uint_32 proflen)
{
	 ...
	 */
	{
		const int result = png_colorspace_set_ICC(png_ptr, &info_ptr->colorspace, name,
	...
	const png_size_t length = strlen(name) + 1;
	png_charp new_iccp_name = png_voidcast(png_charp, png_malloc_warn(png_ptr, length));
	...
	const png_bytep new_iccp_profile = png_voidcast(png_bytep,
               ...
               const png_const_textp text_ptr, const int num_text)
{
	...
	{
		const int old_num_text = info_ptr->num_text;
		png_textp new_text = NULL;

		/* Calculate an appropriate max_text, checking for overflow. */
		int max_text = old_num_text;
	...
	for (int i = 0; i < num_text; i++)
	{
		...
		const size_t key_len = strlen(text_ptr[i].key);
	 ...
	 */
{
	 ...
	 */
	png_sPLT_tp np = png_voidcast(png_sPLT_tp, png_realloc_array(png_ptr,
	...
	do
	{
		 ...
		 */
		const png_size_t length = strlen(entries->name) + 1;
...
add_one_chunk(png_bytep list, unsigned int count, const png_const_bytep add, const int keep)
{
	 ...
	 */
	for (unsigned int i = 0; i < count; ++i, list += 5)
	{
                            ...
                            png_const_bytep chunk_list, const int num_chunks_in)
{
	...
	unsigned int old_num_chunks = png_ptr->num_chunk_list;
