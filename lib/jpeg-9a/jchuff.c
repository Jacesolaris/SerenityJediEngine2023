	...
	c_derived_tbl** pdtbl)
{
	...
	const JHUFF_TBL* htbl = isDC ? cinfo->dc_huff_tbl_ptrs[tblno] : cinfo->ac_huff_tbl_ptrs[tblno];
	if (htbl == NULL)
		ERREXIT1(cinfo, JERR_NO_HUFF_TABLE, tblno);
	 ...
	 */
	const int maxsymbol = isDC ? 15 : 255;
...
{
	const struct jpeg_destination_mgr* dest = state->cinfo->dest;
...
/* Emit some bits; return TRUE if successful, FALSE if must suspend */
{
	...
	register INT32 put_buffer = (INT32)code & ((INT32)1 << size) - 1;

	/* new number of bits in buffer */
	register int put_bits = size + state->cur.put_bits;
	...
	while (put_bits >= 8) {
		const int c = (int)(put_buffer >> 16 & 0xFF);
...
/* Emit some bits, unless we are in gather mode */
{
	...
	/* new number of bits in buffer */
	register int put_bits = size + entropy->saved.put_bits;
...
emit_dc_symbol(huff_entropy_ptr entropy, int tbl_no, int symbol)
{
	if (entropy->gather_statistics)
		...
		const c_derived_tbl* tbl = entropy->dc_derived_tbls[tbl_no];
...
emit_ac_symbol(huff_entropy_ptr entropy, int tbl_no, int symbol)
{
	if (entropy->gather_statistics)
		...
		const c_derived_tbl* tbl = entropy->ac_derived_tbls[tbl_no];
		emit_bits_e(entropy, tbl->ehufco[symbol], tbl->ehufsi[symbol]);
	}
}
...
encode_mcu_DC_first(j_compress_ptr cinfo, JBLOCKROW* MCU_data)
{
	...
	for (int blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
		 ...
		 */
		register int temp = IRIGHT_SHIFT((int)MCU_data[blkn][0][0], cinfo->Al);

		/* DC differences are figured on the point-transformed values. */
		register int temp2 = temp - entropy->saved.last_dc_val[ci];
...
encode_mcu_AC_first(j_compress_ptr cinfo, JBLOCKROW* MCU_data)
{
	...
	register int r = 0;			/* r = run length of zeros */

	for (register int k = cinfo->Ss; k <= Se; k++) {
		...
		/* Find the number of bits needed for the magnitude of the coefficient */
		register int nbits = 1;			/* there must be at least one 1 bit */
		while (temp >>= 1)
			nbits++;
		/* Check for out-of-range coefficient values */
...
encode_mcu_AC_refine(j_compress_ptr cinfo, JBLOCKROW* MCU_data)
{
	...
	char* BR_buffer = entropy->bit_buffer + entropy->BE; /* Append bits to buffer */
...
{
	register int temp2;
	const int Se = state->cinfo->lim_Se;
	const int* natural_order = state->cinfo->natural_order;

	/* Encode the DC coefficient difference per section F.1.2.1 */

	register int temp = temp2 = block[0] - last_dc_val;
	...
	long dc_counts[], long ac_counts[])
{
	...
	register int temp = block[0] - last_dc_val;
	if (temp < 0)
		temp = -temp;
