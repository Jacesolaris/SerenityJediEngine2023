...
/* Allocate a "small" object */
{
	...
	const size_t odd_bytes = sizeofobject % SIZEOF(ALIGN_TYPE);
	if (odd_bytes > 0)
	...
	small_pool_ptr prev_hdr_ptr = NULL;
	small_pool_ptr hdr_ptr = mem->small_list[pool_id];
	while (hdr_ptr != NULL) {
	...
	if (hdr_ptr == NULL) {
		...
		const size_t min_request = sizeofobject + SIZEOF(small_pool_hdr);
	...
	char* data_ptr = (char*)(hdr_ptr + 1); /* point to first data byte in pool */
	...
	/* Allocate a 2-D sample array */
{
	...
	const long ltemp = (MAX_ALLOC_CHUNK - SIZEOF(large_pool_hdr)) /
...
/* Allocate the in-memory buffers for any unrealized virtual arrays */
{
	...
	const long avail_mem = jpeg_mem_available(cinfo, space_per_minheight, maximum_space,
...
/* Do backing store read or write of a virtual sample array */
{
	...
	for (long i = 0; i < (long)ptr->rows_in_mem; i += ptr->rowsperchunk) {
		/* One chunk, but check for short chunk at end of buffer */
		long rows = MIN((long)ptr->rowsperchunk, (long)ptr->rows_in_mem - i);
		/* Transfer no more than is currently defined */
		const long thisrow = (long)ptr->cur_start_row + i;
		rows = MIN(rows, (long)ptr->first_undef_row - thisrow);
		/* Transfer no more than fits in file */
		rows = MIN(rows, (long)ptr->rows_in_array - thisrow);
		if (rows <= 0)		/* this chunk might be past end of file! */
			break;
		const long byte_count = rows * bytesperrow;
		if (writing)
...
free_pool(j_common_ptr cinfo, int pool_id)
{
	...
	if (pool_id == JPOOL_IMAGE) {
		for (jvirt_sarray_ptr sptr = mem->virt_sarray_list; sptr != NULL; sptr = sptr->next) {
		...
		for (jvirt_barray_ptr bptr = mem->virt_barray_list; bptr != NULL; bptr = bptr->next) {
	...
	while (lhdr_ptr != NULL) {
		const large_pool_ptr next_lhdr_ptr = lhdr_ptr->hdr.next;
	...
	while (shdr_ptr != NULL) {
		const small_pool_ptr next_shdr_ptr = shdr_ptr->hdr.next;
...
jinit_memory_mgr(j_common_ptr cinfo)
{
	 ...
	 */
	  ...
	  */
	const size_t test_mac = (size_t)MAX_ALLOC_CHUNK;
