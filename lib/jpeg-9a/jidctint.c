              ...
              const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	wsptr = workspace;
	for (ctr = 0; ctr < 7; ctr++) {
		const JSAMPROW outptr = output_buf[ctr] + output_col;
              ...
              const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	SHIFT_TEMPS
		...
		const ISLOW_MULT_TYPE* quantptr = (ISLOW_MULT_TYPE*)compptr->dct_table;

	/* Column 0 */
	INT32 tmp4 = DEQUANTIZE(coef_block[DCTSIZE * 0], quantptr[DCTSIZE * 0]);
	INT32 tmp5 = DEQUANTIZE(coef_block[DCTSIZE * 1], quantptr[DCTSIZE * 1]);
	/* Add fudge factor here for final descale. */
	tmp4 += ONE << 2;

	const INT32 tmp0 = tmp4 + tmp5;
	const INT32 tmp2 = tmp4 - tmp5;

	/* Column 1 */
	tmp4 = DEQUANTIZE(coef_block[DCTSIZE * 0 + 1], quantptr[DCTSIZE * 0 + 1]);
	tmp5 = DEQUANTIZE(coef_block[DCTSIZE * 1 + 1], quantptr[DCTSIZE * 1 + 1]);

	const INT32 tmp1 = tmp4 + tmp5;
	const INT32 tmp3 = tmp4 - tmp5;

	/* Pass 2: process 2 rows, store into output array. */

	/* Row 0 */
	JSAMPROW outptr = output_buf[0] + output_col;
              ...
              const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	SHIFT_TEMPS
		...
		const ISLOW_MULT_TYPE* quantptr = (ISLOW_MULT_TYPE*)compptr->dct_table;
	int dcval = DEQUANTIZE(coef_block[0], quantptr[0]);
               ...
               const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	for (ctr = 0; ctr < 5; ctr++) {
		...
		const INT32 tmp22 = z3 - ((z1 - z2) << 1);               /* c0 = (c4-c8)*2 */
		...
		const INT32 tmp20 = tmp10 + tmp12;
		const INT32 tmp24 = tmp10 - tmp12;
		const INT32 tmp21 = tmp11 + tmp13;
		const INT32 tmp23 = tmp11 - tmp13;
              ...
              const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	for (ctr = 0; ctr < 4; ctr++) {
		...
		z1 = MULTIPLY(z2 + z3, FIX_0_541196100);       /* c6 */
		tmp2 = z1 + MULTIPLY(z2, FIX_0_765366865);     /* c2-c6 */
		INT32 tmp3 = z1 - MULTIPLY(z3, FIX_1_847759065);     /* c2+c6 */
		...
		tmp0 = (z2 + z3) << CONST_BITS;
		INT32 tmp1 = (z2 - z3) << CONST_BITS;

		tmp10 = tmp0 + tmp2;
		const INT32 tmp13 = tmp0 - tmp2;
		const INT32 tmp11 = tmp1 + tmp3;
		tmp12 = tmp1 - tmp3;
              ...
              const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	for (ctr = 0; ctr < 3; ctr++) {
		...
		tmp2 = (INT32)wsptr[4];
		tmp10 = MULTIPLY(tmp2, FIX(0.707106781));   /* c4 */
		INT32 tmp1 = tmp0 + tmp10;
		const INT32 tmp11 = tmp0 - tmp10 - tmp10;
		tmp10 = (INT32)wsptr[2];
		tmp0 = MULTIPLY(tmp10, FIX(1.224744871));   /* c2 */
		tmp10 = tmp1 + tmp0;
		tmp12 = tmp1 - tmp0;
              ...
              const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	for (ctr = 0; ctr < 2; ctr++) {
		...
		tmp10 = (tmp0 + tmp2) << CONST_BITS;
		const INT32 tmp12 = (tmp0 - tmp2) << CONST_BITS;

		/* Odd part */
		/* Same rotation as in the even part of the 8x8 LL&M IDCT */

		const INT32 z2 = wsptr[1];
		const INT32 z3 = wsptr[3];

		const INT32 z1 = MULTIPLY(z2 + z3, FIX_0_541196100);   /* c6 */
		tmp0 = z1 + MULTIPLY(z2, FIX_0_765366865); /* c2-c6 */
		tmp2 = z1 - MULTIPLY(z3, FIX_1_847759065); /* c2+c6 */
              ...
              const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	SHIFT_TEMPS
		...
		const ISLOW_MULT_TYPE* quantptr = (ISLOW_MULT_TYPE*)compptr->dct_table;
	const JSAMPROW outptr = output_buf[0] + output_col;

	/* Even part */

	INT32 tmp0 = DEQUANTIZE(coef_block[0], quantptr[0]);
	/* Add fudge factor here for final descale. */
	tmp0 += ONE << 2;

	/* Odd part */

	const INT32 tmp1 = DEQUANTIZE(coef_block[1], quantptr[1]);
               ...
               const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	for (ctr = 0; ctr < 5; ctr++, inptr++, quantptr++, wsptr++) {
		...
		/* Add fudge factor here for final descale. */
		z3 += ONE << (CONST_BITS - PASS1_BITS - 1);
		INT32 z4 = DEQUANTIZE(inptr[DCTSIZE * 4], quantptr[DCTSIZE * 4]);
		z1 = MULTIPLY(z4, FIX(1.144122806));         /* c4 */
		z2 = MULTIPLY(z4, FIX(0.437016024));         /* c8 */
		tmp10 = z3 + z1;
		tmp11 = z3 - z2;

		const INT32 tmp22 = RIGHT_SHIFT(z3 - ((z1 - z2) << 1), /* c0 = (c4-c8)*2 */
			CONST_BITS - PASS1_BITS);
              ...
              const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	for (ctr = 4; ctr > 0; ctr--) {
		...
		if (inptr[DCTSIZE * 1] == 0 && inptr[DCTSIZE * 2] == 0 &&
			...
			inptr[DCTSIZE * 7] == 0) {
			/* AC terms all zero */
			const int dcval = DEQUANTIZE(inptr[DCTSIZE * 0], quantptr[DCTSIZE * 0]) << PASS1_BITS;
              ...
              const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	for (ctr = 0; ctr < 3; ctr++, inptr++, quantptr++, wsptr++) {
		...
		tmp10 = MULTIPLY(tmp2, FIX(0.707106781));   /* c4 */
		INT32 tmp1 = tmp0 + tmp10;
		const INT32 tmp11 = RIGHT_SHIFT(tmp0 - tmp10 - tmp10, CONST_BITS - PASS1_BITS);
		tmp10 = DEQUANTIZE(inptr[DCTSIZE * 2], quantptr[DCTSIZE * 2]);
		tmp0 = MULTIPLY(tmp10, FIX(1.224744871));   /* c2 */
		tmp10 = tmp1 + tmp0;
		tmp12 = tmp1 - tmp0;
		...
		const INT32 z1 = DEQUANTIZE(inptr[DCTSIZE * 1], quantptr[DCTSIZE * 1]);
		const INT32 z2 = DEQUANTIZE(inptr[DCTSIZE * 3], quantptr[DCTSIZE * 3]);
		const INT32 z3 = DEQUANTIZE(inptr[DCTSIZE * 5], quantptr[DCTSIZE * 5]);
		tmp1 = MULTIPLY(z1 + z3, FIX(0.366025404)); /* c5 */
		tmp0 = tmp1 + ((z1 + z2) << CONST_BITS);
		tmp2 = tmp1 + ((z3 - z2) << CONST_BITS);
		tmp1 = (z1 - z2 - z3) << PASS1_BITS;
              ...
              const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	for (ctr = 0; ctr < 2; ctr++, inptr++, quantptr++, wsptr++) {
		...
		tmp0 = DEQUANTIZE(inptr[DCTSIZE * 0], quantptr[DCTSIZE * 0]);
		INT32 tmp2 = DEQUANTIZE(inptr[DCTSIZE * 2], quantptr[DCTSIZE * 2]);
		...
		const INT32 z2 = DEQUANTIZE(inptr[DCTSIZE * 1], quantptr[DCTSIZE * 1]);
		const INT32 z3 = DEQUANTIZE(inptr[DCTSIZE * 3], quantptr[DCTSIZE * 3]);
              ...
              const JSAMPARRAY output_buf, const JDIMENSION output_col)
{
	...
	INT32 tmp0 = DEQUANTIZE(coef_block[DCTSIZE * 0], quantptr[DCTSIZE * 0]);
	/* Add fudge factor here for final descale. */
	tmp0 += ONE << 2;

	/* Odd part */

	const INT32 tmp1 = DEQUANTIZE(coef_block[DCTSIZE * 1], quantptr[DCTSIZE * 1]);
