	 ...
	 */
{
	if (datalen >= APP14_DATA_LEN &&
		...
		const unsigned int version = (GETJOCTET(data[5]) << 8) + GETJOCTET(data[6]);
		const unsigned int flags0 = (GETJOCTET(data[7]) << 8) + GETJOCTET(data[8]);
		const unsigned int flags1 = (GETJOCTET(data[9]) << 8) + GETJOCTET(data[10]);
		const unsigned int transform = GETJOCTET(data[11]);
...
METHODDEF(boolean)
get_interesting_appn(const j_decompress_ptr cinfo)
...
METHODDEF(boolean)
save_marker(const j_decompress_ptr cinfo)
                  ...
                  unsigned int length_limit)
{
	 ...
	 */
	const long maxlength = cinfo->mem->max_alloc_chunk - SIZEOF(struct jpeg_marker_struct);
